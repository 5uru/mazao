...
with col2:
    if st.session_state.current_zone:
        ...
        zone_id, zone_name, crop_type, longitude, latitude = get_zone_data(zone.id, zone.name, zone.crop_type, zone.longitude, zone.latitude)



        st.write(f"# {zone_name}")

        temp, humidity, _ = get_weather_data(f"{latitude},{longitude}")  # Use zone's location
        ...
        calendar = calendar(events=calendar_events, custom_css=custom_css, options={"initialView": 'listWeek', "locale": 'fr'})
            ...
            if st.button("Ajouter à l'agenda"):
                for daily_plan in st.session_state.watering_plan["daily_plans"]:
                    ...
                    for watering_event in daily_plan["watering_schedule"]:
                        add_event(
                                name="Arrosage",
                                date=datetime.datetime.combine(date, datetime.datetime.strptime(watering_event["time"], "%H:%M").time()),
                                event_type="green",
                                management_zone_id=zone_id,
                                description=watering_event["description"]
                        )
                st.success("Tous les événements ont été ajoutés à l'agenda avec succès!")
